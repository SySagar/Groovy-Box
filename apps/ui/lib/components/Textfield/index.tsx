...
const textFieldVariants = cva(
    'grv-w-full grv-rounded-md grv-transition-all grv-duration-200 grv-ease-in-out',
    {
        variants: {
            variant: {
                filled: 'grv-bg-secondary-100 focus:bg-primary-50 grv-border-b-2 grv-border-primary focus:grv-outline-secondary-500 focus:grv-outline-2 focus:grv-border-secondary-500 ',
                outlined: 'grv-bg-transparent grv-outline-2 focus:grv-outline-secondary-300  focus:border-primary-500 focus:grv-outline-2 focus:grv-border-secondary-500',
                standard: 'grv-bg-transparent grv-border-none grv-outline-none grv-relative',
            },
            size: {
...
const Root = ({ children, variant, size, error = false, className, ...props }: TextFieldRootProps) => {
    ...
    return (
        <TextFieldContext.Provider value={{ variant, size, error, isFocused, setIsFocused }}>
            <div className="grv-flex grv-flex-col grv-gap-1">
                <motion.div
                    ...
                    {...props}
                >
                    <div className="grv-flex grv-items-center">
                        {children}
                    </div>
                    {variant === 'standard' && (
                        <motion.div
...
const Input = forwardRef<HTMLInputElement, TextFieldInputProps>(
    ({ className, type = 'text', placeholder, disabled, required, readOnly, onFocus, onBlur, ...props }, ref) => {
